...
**Outcome:** New `OrganizationSettingsReadinessCard` (`apps/web/src/features/project/`, ~319 LOC tsx + ~333 LOC CSS) mounted on `ProjectDashboard` immediately after `WorkspaceManagementReadinessCard`. Pure derivation reading `useProjectStore.projects` aggregate. Where `WorkspaceManagementReadinessCard` verdicts the *count and status mix* of projects, this card verdicts the *coherence of org-level settings across them*: are units consistent (metric vs imperial mixing breaks rollups), is jurisdiction narrow enough that grounding tooling has fully-wired data (only US/CA are fully-wired per the projectStore comment; everything else falls back to global Sprint BG datasets), and how strong is shared-metadata coverage (vision / address / province / boundary / description / conservation auth) across projects. Card body: 5-state verdict pill (Settings aligned / Adequate but lean / Mixed unit system / Settings fragmented / Workspace empty — keyed off unitsMixed (both metric and imperial present), distinctCountries vs avgCoverage thresholds, with aligned requiring single-unit + ≤2 countries + avgCoverage ≥0.75), 6-stat headline (projects / countries / fully-wired count / unit system / avg coverage % / "this wired" yes/no), unit-system block with metric/imperial chip pair, jurisdiction block with per-country chip showing `·wired` flag for US/CA, shared-metadata coverage block listing 6 fields with per-row sage/amber/clay tone bar (≥75% sage, ≥40% amber, otherwise clay) and pct badge, settings-flags block (only renders when unitsMixed ∨ ≥3 countries ∨ avgCoverage<0.4 ∨ no fully-wired projects). tsc clean (exit 0). Manifest line 594 flipped partial→done in the same commit. Atlas commit `017e7b2` on `feat/shared-scoring`, pushed (parallel session had advanced HEAD to `a278a05` between rounds — fast-forward succeeded as `a278a05..017e7b2`).

**Carries forward:** Sixth clean round; the §26 P1 trio is now closed (user-management / workspace-management / organization-settings all done). Only `project-ownership-transfer` (planned) and `backup-and-restore` (planned) remain in the §26 P1 set. New parallel-push observed mid-pipeline: I pushed from `5a24b6f` and the remote had advanced to `a278a05` — so a parallel session is shipping alongside. Recently-touched sections to vary away from next round: §1 / §3 / §5 / §6 / §7 / §8 (×2) / §9 / §11 / §13 / §14 / §15 / §16 / §17 (×2) / §18 / §19 (×2) / §20 / §21 / §22 (×2) / §23 (×2) / §24 / §25 (×2) / §26 (×5 — four siblings + this round) / §27 / §29. Natural next directions to vary off §26: §14 line 363 `spiritual-educational-storytelling-layer` (MT partial), §28 lines 657 / 664 / 669 (MT partials — prayer-pavilion siting, staged-vision raw-to-heaven, dawn-sunset viewpoints), §10 / §12 / §13 P1 partials.

**Preview verification:** Skipped — preview server status uncertain between rounds; tsc exit 0 and the card is pure presentation reading existing `useProjectStore` selectors. Logic risk bounded by direct reuse of selector patterns in sibling §26 cards.

---

## [2026-04-27] session | Atlas — §26 WorkspaceManagementReadinessCard

**Objective:** Close manifest §26 line 593 `workspace-management` (P1 partial). User picked candidate 1 from this round's slate. Pre-flight: `useProjectStore` already exposes the full project list with `status` (active/archived/shared/candidate), `projectType`, `createdAt`, `updatedAt` — and a partial-shipped `WorkspaceManagementReadinessCard.module.css` (289 LOC, untracked) was sitting in `apps/web/src/features/project/` waiting on its `.tsx` counterpart. A previous round had laid the CSS but the component was never written. This round filled the orphan.

**Outcome:** New `WorkspaceManagementReadinessCard` (`apps/web/src/features/project/`, ~315 LOC tsx + the orphan ~289 LOC CSS) mounted on `ProjectDashboard` immediately after `AuditLogCard` in the §26 governance trio. Pure derivation reading `useProjectStore.projects` — no extra fetch, no shared math. Card body: 5-state verdict pill (Workspace healthy / Lean but viable / Single-project workspace / Archive sprawl / Workspace empty — keyed off project count, archive ratio vs 0.5, distinct-type count, and recent-edit count: empty when total=0; sparse when total=1; cluttered when archiveRatio≥0.5 ∧ active≤1; healthy when active≥2 ∧ distinctTypes≥2 ∧ recentTouches≥1; otherwise lean), 6-stat headline (projects / active / archived / distinct types / touched·30d / newest edit), status-distribution block with horizontal bar chart per status (active/shared/candidate/archived) and a "this" tag on the row matching the currently-open project, type-mix block sorted by count with the same "this" tag pattern, and a workspace-flags block (only renders when total=1 ∨ archiveRatio≥0.5 ∨ staleProjects>0 ∨ distinctTypes≤1) flagging single-project workspace, archive sprawl, narrow type variety, and projects unedited >180d. tsc clean (exit 0). Manifest line 593 flipped partial→done in the same commit. Atlas commit `5a24b6f` on `feat/shared-scoring`, pushed.

**Carries forward:** Fifth clean round in a row, plus a new failure mode worth logging: the §26 workspace card had its CSS shipped but never its `.tsx` — the inverse of the §22 truth-up case (manifest flipped before card existed) and of the standard "card exists but manifest stale". Pre-flight protocol now needs to handle three independent stale axes: (1) manifest line vs card existence, (2) card existence vs mount point, (3) `.tsx` vs `.module.css`. The orphan CSS pattern is the rarest; a `find apps/web/src -iname "<CardName>*"` catches it cheaply if it's run before designing. Recently-touched sections to vary away from next round: §1 / §3 / §5 / §6 / §7 / §8 (×2) / §9 / §11 / §13 / §14 / §15 / §16 / §17 (×2) / §18 / §19 (×2) / §20 / §21 / §22 (×2) / §23 (×2) / §24 / §25 (×2) / §26 (×4 — three siblings + this round) / §27 / §29. Natural next directions: §14 line 363 `spiritual-educational-storytelling-layer` (MT partial), §28 lines 657 / 664 / 669 (MT partials), §26 line 594 `organization-settings` (P1 partial — last open §26 P1 leaf).

**Preview verification:** Skipped — preview server status uncertain between rounds; tsc exit 0 and the card is pure presentation reading existing `useProjectStore` selectors. Logic risk bounded by direct reuse of selector patterns already in use by sibling §26 cards (`AuditLogCard`, `MetadataManagementCard`).

---

## [2026-04-27] session | Atlas — §26 UserManagementReadinessCard

**Objective:** Close manifest §26 line 592 `user-management` (P1 partial). User picked candidate 2 from this round's slate. Pre-flight: `MembersTab.tsx` already wires the full invite / role-change / remove flow through `useMemberStore` (fetchMembers, fetchMyRole, inviteMember, updateRole, removeMember) — what was missing was a pre-action audit verdicting the project's current membership posture (owner concentration, role coverage, join recency, stale members) before the steward starts editing.

**Outcome:** New `UserManagementReadinessCard` (`apps/web/src/features/collaboration/`, ~283 LOC tsx + ~280 LOC CSS) mounted at the top of `MembersTab` visible content (after the `!isAuthenticated` gate, sibling to the existing invite form / member list / role reference). Pure derivation reading the current `useMemberStore` snapshot — no extra fetch, since `MembersTab` already populates the store on mount. Card body: 5-state verdict pill (Healthy roster / Adequate but lean / Solo project / Single-owner risk / Roster not loaded — keyed off owner count, total members, and collaborator count: empty when total=0; solo when total=1 ∧ owners=1; sparse when owners=1 ∧ total<4; healthy when owners≥2 ∧ collaborators≥1; otherwise adequate), 6-stat headline (members / owners / collaborators / joined·30d / stale·180d+ / newest join), role-distribution block with horizontal bar chart per role and a "you" tag when the viewer's `myRole` matches a row, and a governance-flags block (only renders when ownerCount===1 ∨ collaboratorCount===0 ∨ staleMembers>0) flagging single-owner succession risk, viewer-only collaborator set, and stale-membership signals (>180d). Auth-gate fallback shows "sign in to load roster" when unauthenticated. tsc clean (exit 0). Manifest line 592 flipped partial→done in the same commit. Atlas commit `732e3ed` on `feat/shared-scoring`, pushed.

**Carries forward:** Fourth clean round in a row — manifest stale at HEAD with no parallel pre-flip, sibling readiness-card pattern reusable, action paths (the full invite/role/remove flow) already wired in `MembersTab` so the card's only job is to *verdict* the inputs before the steward acts. The §26 section now has two siblings (`AuditLogCard` + this card) with `MetadataManagementCard` and the workspace/organization siblings still on deck. Recently-touched sections to vary away from next round: §1 / §3 / §5 / §6 / §7 / §8 (×2) / §9 / §11 / §13 / §14 / §15 / §16 / §17 (×2) / §18 / §19 (×2) / §20 / §21 / §22 (×2) / §23 (×2) / §24 / §25 (×2) / §26 (×3 — sibling + this round) / §27 / §29. Natural next directions: §14 line 363 `spiritual-educational-storytelling-layer` (MT partial), §28 lines 657 / 664 / 669 (MT partials), §26 lines 593 / 594 (P1 partials — workspace-management / organization-settings, direct siblings).

**Preview verification:** Skipped — preview server stopped between rounds and was not restarted; tsc exit 0 and the card is pure presentation reading existing `useMemberStore` selectors plus the standard auth-gate guard. Logic risk bounded by direct reuse of memberStore selector patterns already in use by `MembersTab` itself.
