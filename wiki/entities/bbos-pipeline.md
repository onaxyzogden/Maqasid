...
**Redesigned dashboard (preview, 2026-06-04):** A second, visually redesigned pipeline dashboard now coexists with `BbosFullDashboard` behind an OFF-by-default per-device flag (`bbosNewDashboard` / `bbiz_bbos_new_dash`). It lives in `src/components/bbos/pipeline-dashboard/` and renders entirely from a **single adapter seam** — `buildPipelineViewModel({project, bbosFilter, tasks})`. Dark "cosmic" palette is **scoped to `.bpd`** (does not leak globally). Surfaces: 240px stage rail, Stage Overview (attributes/duʿāʾ/gate), Execution View modal (Research/Asset/Execution/Gate-Check; retrospective OPT shows Metrics/Barakah-Health-Index/Restoration-Mandate), Approval Brief modal (7 sections incl. gate radios). `DashboardView.jsx` branches on the flag; Settings exposes it as a "Labs" row. See [decision: bbos-redesigned-dashboard-adapter-shell](../decisions/2026-06-04-bbos-redesigned-dashboard-adapter-shell.md).

**Light mode (2026-06-19):** The redesigned dashboard now **follows the global app theme toggle** (`data-theme` on `<html>`, default light). The dark "cosmic" palette is no longer permanent — a single `[data-theme="light"] .bpd, .bpd-modal-overlay` block in `BbosPipelineDashboard.css` redefines the ~30 `--bpd-*` vars for light surfaces (page gray → cards near-white → inputs white, darkened accent/status hues with pale `*-dim` tints), re-theming every surface (rail/overview/ExecView/ApprovalBrief/primitives) at once because all components read only `var(--bpd-*)`. The default dark block is untouched, so dark mode is byte-for-byte unchanged. No JSX/store/component changes. See [decision: bbos-pipeline-dashboard-light-mode](../decisions/2026-06-19-bbos-pipeline-dashboard-light-mode.md).

**Adapter mock→live (2026-06-04, same day follow-up):** The adapter body is now a **live-store reader** for the *mappable* concepts, stages IDY→RET — stage status/progress, governing attributes, per-stage duʿāʾ (incl. transliteration from live `trans`), spiritual notes, research/asset items (real labels, G-labels, tri-state filed/in_review/pending status, joined field content), and the stage gate (label + verdict-derived status + gate checks). Gate scoring was extracted to a shared **`src/data/bbos/bbos-stage-score.js`** (`scoreStage(stageId, taskMap)`) imported by *both* this adapter and the legacy `BbosFullDashboard` — single source of truth, no drift. The seam signature gained `tasks` (component owns the `useTaskStore` subscription; adapter stays a pure, tested function — `pipeline-dashboard/__tests__/adapter.test.js`, 8 cases). No-live-equivalent concepts stay inert/mock-fed: OPT retrospective (metrics/Barakah-Health-Index/Restoration-Mandate), typed exec-task forms, JSON stage-pack import, cycle-completion/close-cycle, "proceed with conditions", most of the Approval Brief — deferred to the next pass. See [decision: bbos-dashboard-adapter-mock-to-live](../decisions/2026-06-04-bbos-dashboard-adapter-mock-to-live.md).
...
| Date | Event |
|---|---|
| 2026-06-19 | **Center the Stage Overview duʿāʾ block** — follow-up tweak to `BbosStageOverview`'s attribute card. The duʿāʾ lines were split-aligned (Arabic `.bpd-attrcard__dua-ar` right, translation `.bpd-attrcard__dua-tr` defaulting left); both now `text-align: center` (Arabic keeps `direction: rtl` for correct shaping). Two CSS declarations in `BbosPipelineDashboard.css`; theme-agnostic (applies in both light and dark). Verified live via computed-style inspection + screenshot. |
| 2026-06-19 | **Light mode for the redesigned dashboard** — the `bbosNewDashboard` view now follows the global `data-theme` toggle instead of being permanently dark. One additive `[data-theme="light"] .bpd, [data-theme="light"] .bpd-modal-overlay` block in `BbosPipelineDashboard.css` redefines the ~30 `--bpd-*` palette vars for light surfaces (same elevation order as dark — page gray → cards near-white → inputs white; accent/status hues darkened for text legibility, `*-dim` companions flipped to pale tints). Two literal cleanups: `.bpd-parse-result--err` `#e87c7c` → `var(--bpd-red)`, and a light-only `.bpd-modal` box-shadow softener over the light frosted scrim. Re-themes every surface (rail/overview/ExecView/ApprovalBrief/primitives) at once because `palette.js` + all components reference only `var(--bpd-*)`. Default dark block untouched → dark mode byte-for-byte unchanged. No JSX/store/component changes; legacy `BbosFullDashboard` out of scope. Verified light/dark palette resolution via computed-style inspection (screenshot tool was unresponsive — disclosed); ESLint clean. See [decision](../decisions/2026-06-19-bbos-pipeline-dashboard-light-mode.md). |
| 2026-06-07 | **Redesigned-dashboard UI refinements + stage-selection link** (commit `687c976`). Six changes scoped to the `bbosNewDashboard` view only (legacy `BbosFullDashboard` + board/list/gantt untouched): (1) suppress the shared `LevelNavigator` on this view — the dashboard has its own rail; (2) bound the columns so `.bpd-rail__list` + `.bpd-overview` scroll independently instead of the whole page (no CSS change — the rail/overview were already `overflow-y:auto` but never height-bounded; gated the outer scroll wrapper + `minHeight:0` off in `ProjectBoard.jsx`); (3) **color stages by BBOS layer** not status — new `layerVars(layer)` in `palette.js` → `--c`/`--c-dim`, fed by `--bpd-think #C9A05A` / `--bpd-execute #4AB8A8` / `--bpd-reckon #6366F1` tokens (status still read via locked-dim + active pulse + gate colors); (4) hide the background hexagon glyph (`.bpd-orn { display:none }`); (5) 12px minimum font across the dashboard (65 sub-12px declarations → 12px; ≥12px tiers untouched). Plus the **selection↔bbosFilter two-way link**: `BbosPipelineDashboard` now takes `bbosFilter` + `onStageSelect` (passed by `DashboardView`); when controlled, `bbosFilter` is the single source of truth (no effect — avoids `react-hooks/set-state-in-effect`), so a rail click flows up and the header Download/Upload buttons + exported bundle track the selected stage; uncontrolled falls back to local state seeded from the filter. Verified: build + lint + tests green; DOM verification of all five fixes + screenshot of the link in lock-step. |
| 2026-06-05 | **OPT retrospective + Approval Brief wired live (closes the mock→live deferrals).** Adapter gained `buildOptExecution(taskMap, cycle)` — OPT-S1 metrics (CM-1..4 value-only), OPT-A2 BHI normalized 0–10 + computed hero + live `bhiOverallReading`, OPT-S4 filed restoration actions only (no synthesized tags); and `buildBrief(...)` expanding `stage.brief` (covenant readiness, findings via `scoreStage`, constraints [OPT→OPT-S5 hold list; else unmet signals], assets, gate, closing). `BbosApprovalBrief` gate now drives the pipeline: proceed→`advanceBbosStage(next.id)`, halt→`BBOS_REJECTION_REASONS` picker→`rejectBbosPipeline`, conditions inert; guarded on `gate.canAct` (active stage) + valid decision; dashboard root owns the store subscription. **Fixed** `onAdvance` passing the current stage id (no-op); now resolves next stage from `BBOS_STAGES`. Final OPT-stage proceed = "Close Cycle (deferred)". `adapter.test.js` +7 (16 total). Verified: 77 tests green, lint + build green; preview on synthetic OPT project (metrics 72/64/88/31, BHI hero 8.1·Cycle 3·ALIGNED, hold list, gate) — proceed advanced STR→OFR, halt set `rejectedAt`+reason; flag-OFF legacy unchanged; synthetic data cleaned. Still deferred: cycle-completion (`startNewBbosCycle`), proceed-with-conditions store state, operator/client schema fields, typed exec forms. See [decision](../decisions/2026-06-05-bbos-opt-retro-and-brief-live.md). |
| 2026-06-04 | **Adapter mock→live wiring pass (same-day follow-up).** Replaced `buildPipelineViewModel`'s body with a live-store reader for the mappable concepts (status/progress, attributes, duʿāʾ incl. translit, spiritual notes, research/asset items with labels/G-labels/tri-state status/content, gate status + gate checks) for IDY→RET; OPT retrospective + typed exec forms + brief stay inert/mock. Extracted `STAGE_SCORE_SIGNALS` + scoring into shared `src/data/bbos/bbos-stage-score.js` (`scoreStage`) imported by both the new adapter and legacy `BbosFullDashboard`. Seam signature gained `tasks` arg; `BbosPipelineDashboard.jsx` now subscribes to `useTaskStore`. Added `pipeline-dashboard/__tests__/adapter.test.js` contract guard (8 tests). Verified: 70 tests green, lint + build green, preview screenshots flag ON (live rail/overview/tasks at stage CRD) and flag OFF (legacy regression unchanged). See [decision](../decisions/2026-06-04-bbos-dashboard-adapter-mock-to-live.md). |
