...
- **Phase 2c (deferred):** ~7 thick pages still gate themselves in-body. Clusters: **sources** (`FivePillars`, `HadithPage`, `IslamicKnowledgePage`, `QuranPage`), **dynamic** (`ModulePlaceholder`, `ComingSoon` — `moduleId` from `useParams`; needs extension of `CeremonyGuard` or a sibling dynamic-guard component), **route-id-mismatch** (`Project.jsx` gates `"work"` not `"project"` — needs explicit mapping or pathed inheritance).
