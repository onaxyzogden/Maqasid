...
- **SSR loader:** Vite `createServer({ appType:'custom', server:{ middlewareMode:true, hmr:false, watch:null }, optimizeDeps:{ noDiscovery:true } })` + `ssrLoadModule`, so the app's extensionless / aliased imports resolve exactly as at runtime — a bare `node --import` cannot load these modules.
  > [!note] Amended 2026-07-26: the loader now also passes `configFile: false` and imports `vite.config.js` directly to reuse only its `resolve.alias` map. Letting `createServer` load the app config made every SSR fetch pathologically slow (10–60s/module, tripping the module runner's fixed 60s transport timeout — the recurring red `--check` after this ADR shipped). See [[2026-07-26-milos-mojibake-repair-glyphs-loader-fix]].
