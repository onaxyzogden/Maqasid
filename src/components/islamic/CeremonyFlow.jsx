...
// The full interactive ceremony flow — step tabs (Dua → Allah ﷻ → Ready?/
// Reflection → Bismillah/Alhamdulillah, plus the conditional Pause / Closing
// Dua steps), body renderers, footer, and citations. Extracted verbatim from
// ThresholdModal (2026-07-27) so the ceremony can render both as the global
// overlay modal AND inline inside the Prophetic Path node popup's Before/After
// tabs. The `.thr-*` classes are flat (not scoped to .thr-modal), so the same
// stylesheet dresses both hosts.
//
// Stateless toward the stores: the host resolves `moduleId`, decides what
// completion means (`onComplete` → threshold-store completeOpening/-Closing),
// and resets a finished flow by remounting (key bump) — this component never
// resets its own step state.
...
// ceremony can reuse the same interactive card-pair wizard as opening.
...
function getPauseLabel(rows, selections) {
  ...
  return 'Not yet aligned with the ceremony';
}

export default function CeremonyFlow({ moduleId, type, onComplete }) {
  ...
  // closing ceremony renders the same interactive card-pair wizard as opening.
  ...
  // Closing Dua always appears on the closing ceremony (whether the user
  ...
  const handleSkip = () => {
    const label = isOpening ? 'Opening Ceremony' : 'Closing Ceremony';
    if (window.confirm(`Skip the ${label}? It will be marked complete without going through the steps.`)) {
      complete();
    }
  };
