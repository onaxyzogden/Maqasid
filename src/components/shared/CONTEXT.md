...
| SubtaskStepDetail.jsx | One subtask ("step") rendered Orientation-style: crumb → task title/progress → tag row (priority / Amanah tier / Grounded) → Now box → "Why & how" accordion → Evidence accordion; `subtask == null` renders the "nothing left in this task today" complete state. **Renders a Fragment** — host body must supply flex-column + gap. Props `{ crumbParts, task, subtask, taskStats }`, purely presentational | OrientationSheet, NodePhaseSlideUp (drill-in) |
