...
// Simple inline store using localStorage
function loadPostings() { return safeGetJSON('recruit_postings', []); }
function savePostings(p) { safeSet('recruit_postings', p); }

let _listeners = new Set();
function notify() { _listeners.forEach((fn) => fn()); }

function useRecruitStore() {
  ...
  return useMemo(() => ({
    ...
    move: (id, newStage) => {
      const postings = loadPostings().map((p) =>
    ...
    remove: (id) => {
      savePostings(loadPostings().filter((p) => p.id !== id));
      notify();
    },
  }), []);
}
...
function generatePostingLink(posting) {
