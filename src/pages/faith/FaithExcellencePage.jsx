...
const BOARD_ID = 'faith_excellence';

export default function FaithExcellencePage() {
  const ensureFaithProjects = useProjectStore((s) => s.ensureFaithProjects);
  const project = useProjectStore((s) => s.getProject(BOARD_ID));
  const hasCompletedOpening = useThresholdStore((s) => !!s.completedOpening['faith-excellence']);
  ...
  if (!hasCompletedOpening) {
    return <CeremonyGate moduleId="faith-excellence" />;
  }
