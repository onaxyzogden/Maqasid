...
### 14. Verify against the deployed artifact before scoping a removal
**Finding:** Asked to remove MIOS online signup, orientation from `wiki/` said the feature was dark — the 2026-06-05 log entry still listed "add the two Supabase secrets to GitHub repo settings" as an *outstanding operator action*. Loading the live app showed otherwise: the nav rendered `<a href="/auth">Sign In</a>` (which only renders when `isSupabaseConfigured` is true, i.e. the secrets *were* added) and `/auth` served a working signup form. Planning from the wiki alone would have produced a plan for a feature that wasn't live, and missed that real visitors could create accounts against an unfinished backend.
**Lesson:** The wiki records what was *intended* at write time; only the deployed artifact records what is *true now*. For any "remove/disable X from the live app" task, load the live app and confirm X is actually reachable before scoping. A wiki entry describing an outstanding manual step is the weakest possible evidence that the step is still outstanding — nobody goes back to tick it off. Same class of check applies to feature flags, CI secrets, and DNS.

### 15. Confirm which domain is live — don't infer it from repo files
**Finding:** Opened `bboss.ogden.ag` to verify the live app; operator corrected: *"wrong page. I am referring to maqasid.ogden.ag"*. The guess came from `public/CNAME`, which says `bboss.ogden.ag` — the repo file and the actual deployment disagree, and the file is still that way.
**Lesson:** `public/CNAME`, `vite.config` `base`, and README badges are all *claims* about deployment, not deployment. Ask or confirm the live URL before verifying against it. And when a discrepancy like this surfaces, **flag it, don't fix it** — editing `CNAME` can take a live GitHub Pages site down; that is an operator decision, not a drive-by cleanup.
