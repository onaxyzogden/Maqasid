#!/usr/bin/env node
// Audit: round-trip 10 random Environment subtasks against pre-migration markdown.

const { ENVIRONMENT_SEED_TASKS } = await import(
  new URL('../src/data/seed-tasks/environment-seed-tasks.js', import.meta.url).href,
);
const legacyMod = await import(
  new URL('../tmp/environment-original.js', import.meta.url).href,
);
const LEGACY = legacyMod.ENVIRONMENT_SEED_TASKS;
...
const cur = flatten(ENVIRONMENT_SEED_TASKS);
