// One-shot: build prayer-worklist.md listing every subtask that is
...
const prayer = walk('prayer', all.prayer);

const ratNote = prayer.filter(s => s.ratNote);
const single = prayer.filter(s => s.sources.length === 1);
...
for (const item of worklist) {
  item.refStatus = item.sources.map(s => {
    ...
    const otherPillars = [...pillars].filter(p => p !== 'prayer');
    return {
      ref: s.ref,
      kind: s.kind,
...
const lines = [];
lines.push('# Prayer Grounding Worklist');
lines.push('');
lines.push(`Generated 2026-05-03. Source: \`src/data/seed-tasks/prayer-seed-tasks.js\` (${prayer.length} subtasks total).`);
lines.push('');
lines.push(`- ratNote-flagged: **${ratNote.length}**`);
lines.push(`- single-source:   **${single.length}**`);
lines.push(`- unique union:    **${worklist.length}**`);
lines.push('');
lines.push('Cross-pillar dedup: any worklist source whose `ref` already appears in another pillar (Faith, Prayer, Health, Intellect, Family, Wealth, Environment, Prayer) is tagged `[verified-elsewhere]` — these can skip a fresh NotebookLM probe and inherit the prior verdict.');
lines.push('');
lines.push('---');
lines.push('');
...
await writeFile(resolve(outDir, 'prayer-worklist.md'), lines.join('\n') + '\n', 'utf8');
console.log(`Wrote tasks/prayer-worklist.md`);
