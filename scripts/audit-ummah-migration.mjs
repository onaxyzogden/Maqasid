#!/usr/bin/env node
// Audit: round-trip 10 random Ummah subtasks against pre-migration markdown.

const { UMMAH_SEED_TASKS } = await import(
  new URL('../src/data/seed-tasks/ummah-seed-tasks.js', import.meta.url).href,
);
const legacyMod = await import(
  new URL('../tmp/ummah-original.js', import.meta.url).href,
);
const LEGACY = legacyMod.UMMAH_SEED_TASKS;
...
const cur = flatten(UMMAH_SEED_TASKS);
