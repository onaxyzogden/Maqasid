#!/usr/bin/env node
// Audit: round-trip 10 random Intellect subtasks against pre-migration markdown.

const { INTELLECT_SEED_TASKS } = await import(
  new URL('../src/data/seed-tasks/intellect-seed-tasks.js', import.meta.url).href,
);
const legacyMod = await import(
  new URL('../tmp/intellect-original.js', import.meta.url).href,
);
const LEGACY = legacyMod.INTELLECT_SEED_TASKS;
...
const cur = flatten(INTELLECT_SEED_TASKS);
