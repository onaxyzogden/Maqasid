// One-shot: build moontrance-worklist.md listing every subtask that is
...
const moontrance = walk('moontrance', all.moontrance);

const ratNote = moontrance.filter(s => s.ratNote);
const single = moontrance.filter(s => s.sources.length === 1);
...
for (const item of worklist) {
  item.refStatus = item.sources.map(s => {
    ...
    const otherPillars = [...pillars].filter(p => p !== 'moontrance');
    return {
      ref: s.ref,
      kind: s.kind,
...
const lines = [];
lines.push('# Moontrance Grounding Worklist');
lines.push('');
lines.push(`Generated 2026-05-03. Source: \`src/data/seed-tasks/moontrance-seed-tasks.js\` (${moontrance.length} subtasks total).`);
lines.push('');
...
await writeFile(resolve(outDir, 'moontrance-worklist.md'), lines.join('\n') + '\n', 'utf8');
console.log(`Wrote tasks/moontrance-worklist.md`);
