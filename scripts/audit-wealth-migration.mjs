#!/usr/bin/env node
// Audit: round-trip 10 random Wealth subtasks against pre-migration markdown.

const { WEALTH_SEED_TASKS } = await import(
  new URL('../src/data/seed-tasks/wealth-seed-tasks.js', import.meta.url).href,
);
const legacyMod = await import(
  new URL('../tmp/wealth-original.js', import.meta.url).href,
);
const LEGACY = legacyMod.WEALTH_SEED_TASKS;
...
const cur = flatten(WEALTH_SEED_TASKS);
