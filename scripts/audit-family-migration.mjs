#!/usr/bin/env node
// Audit: round-trip 10 random Family subtasks against pre-migration markdown.

const { FAMILY_SEED_TASKS } = await import(
  new URL('../src/data/seed-tasks/family-seed-tasks.js', import.meta.url).href,
);
const legacyMod = await import(
  new URL('../tmp/family-original.js', import.meta.url).href,
);
const LEGACY = legacyMod.FAMILY_SEED_TASKS;
...
const cur = flatten(FAMILY_SEED_TASKS);
